...
BPF_HASH(sock_wmem, struct sock*, u64);
...
{
    submit_tcp_data((struct sock*)req, ctx, "kprobe:tcp_v4_syn_recv_sock");
    return 0;
}
...
int kretprobe__tcp_v4_syn_recv_sock(struct pt_regs* ctx)
{
    ...
    if  (submit_tcp_data(sk, ctx, "kretprobe:tcp_v4_syn_recv_sock") < 0)
        return 0;

    u64 val = 0;
    sock_wmem.update(&sk, &val);
    return 0;
}
...
int kprobe__tcp_done(struct pt_regs* ctx, struct sock *sk)
{
    ...
    submit_tcp_data_with_two_args(sk, ctx, "kprobe:tcp_done", val, 0);
    return 0;
}
...
int kretprobe__sk_stream_alloc_skb(struct pt_regs* ctx)
{
    ...
    if  (skpp)
    {
        ...
        submit_tcp_data_with_two_args(sk, ctx, "kretprobe:sk_stream_alloc_skb", skb->truesize, val);
...
int kprobe____kfree_skb(struct pt_regs* ctx, struct sk_buff *skb)
{
    ...
    if  (skpp)
    {
        ...
        submit_tcp_data_with_two_args(sk, ctx, "kprobe:__kfree_skb", skb->truesize, val);
    }
