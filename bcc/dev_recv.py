...
class data_common(ct.Structure):
    ...
    def print(self):
        print("%-8s %-9.6f  %4d %7d %7d %-15.15s %-30s " % (
                # self.type, 
                strftime("%H:%M:%S"), float(self.ts_us) / 1e6, 
                self.cpu_id, self.tid, self.pid, self.task, self.function),
            end=""
        )
...
class napi_data(ct.Structure):
    _fields_ = [
        ("common", data_common),
        ("napi_addr", ct.c_ulong),
        ("napi_id", ct.c_uint),
        ("blank", ct.c_uint),
        ("work", ct.c_int),
        ("budget", ct.c_int),
        ("dev_name", ct.c_char* IFNAMESIZE)
    ]
    def print(self):
        self.common.print()
        print("dev_name: %s, napi_id: %d, work: %d, budget: %d" % (
            self.dev_name, self.napi_id, self.work, self.budget), end="")
...
def print_data_event(cpu, event, size):
    ...
    if  type == 1:
        ...
        ct.cast(event, ct.POINTER(napi_data)).contents.print()
