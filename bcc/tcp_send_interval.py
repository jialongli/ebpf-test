...
# define BPF program
bpf_file = open("tcp_send_interval.c")
...
class data_common(ct.Structure):
    ...
    def print(self):
        print("%2d %-8s %-9.6f  %7d %7d %-15.15s %-32s " % (
                self.type, strftime("%H:%M:%S"), float(self.ts_us) / 1e6, 
...
class tcp_rcv_established_data(ct.Structure):
    ...
    def print(self):
        ...
        print("    seg_seq:          0x%08x" % self.seg_seq)
        print("    seg_len:          0x%08x" % self.seg_len)
        print("    seg_ack:          0x%08x" % self.seg_ack)
        print("    seg_wnd:          0x%08x" % self.seg_wnd)
...
class tcp_transmit_skb_data(ct.Structure):
    ...
    def print(self):
        ...
        print("    rcv_nxt:          0x%08x" % self.seg_ack)
...
class tcp_data_with_two_args(ct.Structure):
    ...
    def print(self):
        self.tcp_common.print()
        print("arg1: %d, arg2: %d " % (self.arg1, self.arg2))
...
class tcp_data_with_poll_event(ct.Structure):
    ...
    def print(self):
        ...
        print("ret: 0x%08x, POLLIN: %d, POLLOUT: %d, POLLERR: %d, POLLHUP: %d" % (
                self.event,
...
class tcp_recvmsg_data(ct.Structure):
    _fields_ = [
        ("tcp_common", tcp_data),
    ]
    ...
    def print(self):
        self.tcp_common.print()
        print("recvmsg_interval: %lf " % ((self.tcp_common.common.ts_us - tcp_recvmsg_data.last_ts) / 1e6))
        tcp_recvmsg_data.last_ts = self.tcp_common.common.ts_us



class tcp_sendmsg_data(ct.Structure):
    _fields_ = [
        ("tcp_common", tcp_data),
    ]
    ...
    def print(self):
        self.tcp_common.print()
        print("sendmsg_interval: %lf " % ((self.tcp_common.common.ts_us - tcp_sendmsg_data.last_ts) / 1e6))
        tcp_sendmsg_data.last_ts = self.tcp_common.common.ts_us
...
def print_data_event(cpu, event, size):
    ...
    if  type == 1:
        ...
        ct.cast(event, ct.POINTER(tcp_recvmsg_data)).contents.print()
    elif type == 12:
        ct.cast(event, ct.POINTER(tcp_sendmsg_data)).contents.print()
