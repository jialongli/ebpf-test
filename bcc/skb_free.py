...
class sock_data_with_stack(ct.Structure):
    _fields_ = [
        ("sock", sock_data),
        ("stack_id", ct.c_uint),
        ("blank",    ct.c_uint),
    ...
    def print(self):
        self.sock.print()
        print("%d, %d" % (self.stack_id, self.blank))
        # print("%s -> %s" % (tcp_states[self.arg1], tcp_states[self.arg2]), end="")
        if self.stack_id != 0:
            for addr in stack_traces.walk(self.stack_id):
...
def print_data_event(cpu, event, size):
    ...
    if  type == 1:
        ...
        ct.cast(event, ct.POINTER(sock_data)).contents.print()
    # elif type == 3:
    ...
    #     ct.cast(event, ct.POINTER(tcp_data_with_state_trans)).contents.print()
        ...
        ct.cast(event, ct.POINTER(sock_data_with_stack)).contents.print()
